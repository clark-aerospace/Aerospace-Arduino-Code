...
unsigned long startTime=0;//for the reset switch
...
float fakeAltRead(){//for debug and demo purposes entierly temp
  int initialV=275;
  if ((millis()-startTime)<2000)
    return (float)groundAltitude;
  float testtime = (((millis()-startTime))/1000.0);
  Serial.print((String)(testtime)+",");
  Serial.println((String)(groundAltitude+(initialV*testtime)-(4.9*testtime*testtime)));
...
float getSpeedfromAltitude(){//this function will get the speed from the BME280 altitude sense; changes lastReadAltitude and lastReadTime; m/s
  float altRead = fakeAltRead();  //gets altitude in meters //
